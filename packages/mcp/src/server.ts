...
Pass a sessionKey from a previous task to continue the same thread.`,
      inputSchema: z.object({
        ...
        sessionKey: z
          .string()
          .optional()
          .describe("Session key from a previous task to continue the same thread"),
        senderName: z
          .string()
          .optional()
          .describe(
      ...
      _meta: provider.toolMeta?.run_task,
    },
...
completed_no_summary and error are terminal — report them. Rarely, a long tool-heavy run finishes its answer after the connector marks it terminal; one follow-up check_task ~30s later can upgrade such a job to completed. Do that at most once or twice, not as routine.`,
      inputSchema: z.object({
        ...
        sessionKey: z
          ...
          .optional()
          .describe(
            "The sessionKey from run_task — resolves the session's latest job. Alternative to jobId.",
          ),
        agent: agentEnum
          .optional()
          .describe(
            "Usually inferred from jobId; set only if run_task ran in a different process.",
          ),
        knownLogCount: z
          .number()
          .optional()
  ...
  server.registerTool(
    "list_tasks",
    {
      ...
      annotations: { readOnlyHint: true, idempotentHint: true, openWorldHint: false },
      _meta: provider.toolMeta?.list_tasks,
    },
    async ({ view }) => {
      const tasks = listTasks(pool);
      const filtered =
        view === "active" ? tasks.filter((t) => isActiveTaskStatus(t.status)) : tasks;
...
At detail levels that include it, \`updates\` is a bounded recent-activity window, not the full accumulated log — pass \`knownLogCount\` to get only newer entries. \`summary\`/\`artifacts\` are never bounded by the cursor: once terminal they are always complete.`,
      inputSchema: z.object({
        taskId: z
          .string()
          .describe("Task identifier. Same value as jobId — either name resolves the same task."),
        detail: z
          .enum([
            "core",
            "summary",
      ...
      annotations: { readOnlyHint: true, idempotentHint: true, openWorldHint: false },
      _meta: provider.toolMeta?.get_task,
    },
    async ({ taskId, detail, knownLogCount }) => {
      ...
      if (d === "prompt") {
        ...
        return {
          content: [{ type: "text", text: JSON.stringify({ taskId, prompt: result.prompt }) }],
          structuredContent: { taskId, prompt: result.prompt },
        };
      }
    ...
    "get_session",
    {
      ...
      inputSchema: z.object({
        ...
        limit: z
          ...
          .max(200)
          .optional()
          .describe("Max events per page for events/tail modes. Default 50, max 200."),
        after: z
          .number()
          .int()
          .nonnegative()
      ...
      annotations: { readOnlyHint: true, idempotentHint: true, openWorldHint: false },
      _meta: provider.toolMeta?.get_session,
    },
  ...
  server.registerTool(
    "list_sessions",
    {
      description: `List every OpenClaw session this connector knows about, across agents — including finished ones, since a completed session's sessionKey is what you pass to run_task to continue that thread. Shows agent, session key, last job, last summary, and a recommended next step.`,
      inputSchema: z.object({}),
      annotations: { readOnlyHint: true, idempotentHint: true, openWorldHint: false },
      _meta: provider.toolMeta?.list_sessions,
    },
    async () => {
  ...
  server.registerTool(
    "list_agents",
    {
      ...
      inputSchema: z.object({}),
      annotations: { readOnlyHint: true, idempotentHint: true, openWorldHint: false },
      _meta: provider.toolMeta?.list_agents,
    },
    async () => {
    ...
    "search_memory",
    {
      ...
      inputSchema: z.object({
        ...
        collections: z
          ...
          .optional()
          .describe(
            "Restrict to these collection names. Omit to search all collections the connection can reach.",
          ),
        intent: z
          .string()
          .optional()
          .describe(
            "One-line description of why you're searching — for telemetry only, not sent to QMD.",
          ),
      }),
      annotations: { readOnlyHint: true, idempotentHint: true, openWorldHint: false },
      _meta: provider.toolMeta?.search_memory,
    },
    async ({ query, limit, collections, intent }) => {
      const result = await searchMemory(config.registry.agents, {
        query,
        limit,
        collections,
        intent,
  ...
  server.registerTool(
    "get_memory",
    {
      ...
      annotations: { readOnlyHint: true, idempotentHint: true, openWorldHint: false },
      _meta: provider.toolMeta?.get_memory,
    },
    async ({ file }) => {
  ...
  server.registerTool(
    "list_collections",
    {
      ...
      inputSchema: z.object({}),
      annotations: { readOnlyHint: true, idempotentHint: true, openWorldHint: false },
      _meta: provider.toolMeta?.list_collections,
    },
    async () => {
