...
**Independent lifecycle state.** Task snapshots expose `execution`, `upstream`, `transcript`, and `cancellation` separately. Live progress uses OpenClaw's session-message subscription plus the stable sequence/message identities in `chat.history`: after a socket loss the connector replays everything after `lastSeenSequence`, deduplicates it, resubscribes, and runs a second catch-up read before returning `transcript: "live"`. A detached transcript never implies that execution stopped. The older final-text poll remains only as a last-resort completion fallback.
...
- In-flight jobs survive a connector restart: each agent's currently-`running` jobs (jobId/sessionKey/prompt plus the confirmed upstream run/transcript cursors — no logs, nothing terminal) are written to `apps/chatgpt/.job-store/<agentId>.json` and reloaded on boot, then reattached through durable transcript replay. A job that was already terminal before the restart is never written, so the file can't grow into a log; delete the directory any time to reset to pre-persistence behavior (a restart during that job just re-derives from scratch instead of reattaching)
...
As of the resumable-transcript implementation: **31 test files, 597 tests, all passing.** Coverage by client surface:
...
| Resumable OpenClaw transcript | Real WebSocket disconnect/reconnect: ordered durable tool replay after the confirmed sequence, stable-id dedupe, connection-scoped resubscription, post-subscribe race catch-up, normal completion on the original run, and `chat.send` acknowledgement retry with the same idempotency key | `packages/core/src/transcript-recovery.test.ts` |
